...
mod ssr {
    use frender_html::dom::attr_value::ssr::SsrAttrValue;

    use super::Absent;

    impl<VK: ?Sized> SsrAttrValue<VK> for Absent {
...
mod csr {
    use frender_html::dom::attr_value::csr::{CsrAttrValue, ValueKind};

    use super::Absent;

    impl<VK: ?Sized + ValueKind> CsrAttrValue<VK> for Absent {
        type State = ();

...
mod event_listener {
    ...
    impl<R: ?Sized, ET: frender_dom::event_types::EventType, E: ?Sized + OnEvent<R, ET>> OnEvent<R, ET> for ElementProxyAttrs<E> {
        type EventListener<F: HandleEvent<<ET as frender_dom::event_types::EventType>::Event> + 'static> = EventListener<E::EventListener<F>>;
        type EventListenerUnpinned<F: HandleEvent<<ET as frender_dom::event_types::EventType>::Event> + 'static> = EventListener<E::EventListenerUnpinned<F>>;
    }
}

mod dom {
    use frender_dom::csr::{behaviors, render::RenderWithContext, render_from::str::ValueForStr};
    ...
    impl<R: ?Sized, E: ?Sized + behaviors::SetInnerHtmlFromStr<R>> behaviors::SetInnerHtmlFromStr<R> for ElementProxyAttrs<E> {
        fn set_inner_html_from_str(&mut self, renderer: &mut R, value: impl ValueForStr) {
    ...
    impl<R: ?Sized, E: ?Sized + behaviors::SetInnerTextFromStr<R>> behaviors::SetInnerTextFromStr<R> for ElementProxyAttrs<E> {
        fn set_inner_text_from_str(&mut self, renderer: &mut R, value: impl ValueForStr) {
...
mod form_control {
    ...
    impl<
            //
            E: ?Sized + FormControlElement<VK, Renderer>,
            VK: ?Sized + FormControlValueKind,
            Renderer: ?Sized,
        > FormControlElement<VK, Renderer> for ElementProxyAttrs<E>
    {
        ...
        fn remove_default_value(&mut self, renderer: &mut Renderer) {
            self.0.remove_default_value(renderer)
        }
        ...
        type OnValueChangeEventListenerUnpinned<F: HandleFormControlValue<VK> + 'static> = E::OnValueChangeEventListenerUnpinned<F>;
        ...
        type OnValueChangeFUnpinned<F: HandleFormControlValue<VK> + 'static> = E::OnValueChangeFUnpinned<F>;
    }
}
...
pub(crate) mod macros {
    macro_rules! impl_behavior_fn {
        ...
        ($fn_name:ident ($value:ident : attr_value![$maybe_ty:ty]) {
            ...
            crate::element_proxy_attrs::csr::macros::impl_behavior_fn_update_with! {
    ...
    macro_rules! impl_behavior_fn_update_with {
        ...
        (
            ...
            fn $set_attribute_ident(&mut self, renderer: &mut Renderer, $value: $maybe_ty) {
                crate::element_proxy_attrs::csr::proxy_attr::ProxyAttribute::proxy_attribute(self, renderer, $attr_name, $value)
            }
        };
        (

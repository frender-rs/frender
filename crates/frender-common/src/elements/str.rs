...
use crate::{
    renderer::{self, Text},
    Element, HtmlRenderer, RenderState,
};
...
impl<Cache, R: HtmlRenderer> State<Cache, R::Text> {
    fn add_self_to_dom(&self, renderer: &mut R, force_reposition: bool) {
        renderer.readd_text(text, force_reposition || self.unmounted)
    }
...
impl<Cache, Text: renderer::Text> Unpin for State<Cache, Text> {}

impl<Cache, Text: renderer::Text> RenderState for State<Cache, Text> {
...
macro_rules! impl_render_str {
    ($(
        ...
        impl $(<$($generics)*>)? Element for $for_ty {
            type RenderState<Renderer: HtmlRenderer> = State<<Self as RenderingStr>::Cache, Renderer::Text>;

            fn render_into<Renderer: HtmlRenderer>(
                self,
                renderer: &mut Renderer,
            ) -> Self::RenderState<Renderer> {
                Self::RenderState::<Renderer>::initialize_with_str(self, renderer)
            }

            fn render_update_maybe_reposition<Renderer: HtmlRenderer>(
                self,
                renderer: &mut Renderer,
                render_state: std::pin::Pin<&mut Self::RenderState<Renderer>>,
                force_reposition: bool,
            ) {
                render_state.get_mut().update_with_str_maybe_reposition(self, renderer, force_reposition)
            }
        }
    )*};
}

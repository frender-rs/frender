macro_rules! define_element_trait {
    (
        ...
        $vis use crate::html::behaviors::$WithEventsTrait;
...
define!(
    pub trait Element {
        ...
        const on_security_policy_violation: SecurityPolicyViolationEvent = (
            "securitypolicyviolation",
            OnSecurityPolicyViolationEvent,
            OnSecurityPolicyViolationEventListener,
        );
        ...
        const on_composition_end: CompositionEvent = (
            "compositionend",
            OnCompositionEndEvent,
            OnCompositionEndEventListener,
        );
        const on_composition_start: CompositionEvent = (
            "compositionstart",
            OnCompositionStartEvent,
            OnCompositionStartEventListener,
        );
        const on_composition_update: CompositionEvent = (
            "compositionupdate",
            OnCompositionUpdateEvent,
            OnCompositionUpdateEventListener,
        );
        ...
        const on_fullscreen_change: Event = (
            "fullscreenchange",
            OnFullscreenChangeEvent,
            OnFullscreenChangeEventListener,
        );
        const on_fullscreen_error: Event = (
            "fullscreenerror",
            OnFullscreenErrorEvent,
            OnFullscreenErrorEventListener,
        );
        ...
        const on_context_menu: MouseEvent = (
            "contextmenu",
            OnContextMenuEvent,
            OnContextMenuEventListener,
        );
        const on_double_click: MouseEvent =
            ("dblclick", OnDoubleClickEvent, OnDoubleClickEventListener);
        const on_mouse_down: MouseEvent = ("mousedown", OnMouseDownEvent, OnMouseDownEventListener);
        const on_mouse_enter: MouseEvent =
            ("mouseenter", OnMouseEnterEvent, OnMouseEnterEventListener);
        const on_mouse_leave: MouseEvent =
            ("mouseleave", OnMouseLeaveEvent, OnMouseLeaveEventListener);
        ...
        const on_touch_cancel: TouchEvent = (
            "touchcancel",
            OnTouchCancelEvent,
            OnTouchCancelEventListener,
        );
        ...
        const on_touch_start: TouchEvent =
            ("touchstart", OnTouchStartEvent, OnTouchStartEventListener);

        fn sub_traits() {
            pub trait HtmlElement {
                const on_invalid: Event = ("invalid", OnInvalidEvent, OnInvalidEventListener);
                const on_animation_cancel: AnimationEvent = (
                    "animationcancel",
                    OnAnimationCancelEvent,
                    OnAnimationCancelEventListener,
                );
                const on_animation_end: AnimationEvent = (
                    "animationend",
                    OnAnimationEndEvent,
                    OnAnimationEndEventListener,
                );
                const on_animation_iteration: AnimationEvent = (
                    "animationiteration",
                    OnAnimationIterationEvent,
                    OnAnimationIterationEventListener,
                );
                const on_animation_start: AnimationEvent = (
                    "animationstart",
                    OnAnimationStartEvent,
                    OnAnimationStartEventListener,
                );
                const on_before_input: InputEvent = (
                    "beforeinput",
                    OnBeforeInputEvent,
                    OnBeforeInputEventListener,
                );
                ...
                const on_got_pointer_capture: PointerEvent = (
                    "gotpointercapture",
                    OnGotPointerCaptureEvent,
                    OnGotPointerCaptureEventListener,
                );
                const on_lost_pointer_capture: PointerEvent = (
                    "lostpointercapture",
                    OnLostPointerCaptureEvent,
                    OnLostPointerCaptureEventListener,
                );
                const on_pointer_cancel: PointerEvent = (
                    "pointercancel",
                    OnPointerCancelEvent,
                    OnPointerCancelEventListener,
                );
                const on_pointer_down: PointerEvent = (
                    "pointerdown",
                    OnPointerDownEvent,
                    OnPointerDownEventListener,
                );
                const on_pointer_enter: PointerEvent = (
                    "pointerenter",
                    OnPointerEnterEvent,
                    OnPointerEnterEventListener,
                );
                const on_pointer_leave: PointerEvent = (
                    "pointerleave",
                    OnPointerLeaveEvent,
                    OnPointerLeaveEventListener,
                );
                const on_pointer_move: PointerEvent = (
                    "pointermove",
                    OnPointerMoveEvent,
                    OnPointerMoveEventListener,
                );
                const on_pointer_out: PointerEvent =
                    ("pointerout", OnPointerOutEvent, OnPointerOutEventListener);
                const on_pointer_over: PointerEvent = (
                    "pointerover",
                    OnPointerOverEvent,
                    OnPointerOverEventListener,
                );
                const on_pointer_up: PointerEvent =
                    ("pointerup", OnPointerUpEvent, OnPointerUpEventListener);
                const on_transition_cancel: TransitionEvent = (
                    "transitioncancel",
                    OnTransitionCancelEvent,
                    OnTransitionCancelEventListener,
                );
                const on_transition_end: TransitionEvent = (
                    "transitionend",
                    OnTransitionEndEvent,
                    OnTransitionEndEventListener,
                );
                const on_transition_run: TransitionEvent = (
                    "transitionrun",
                    OnTransitionRunEvent,
                    OnTransitionRunEventListener,
                );
                const on_transition_start: TransitionEvent = (
                    "transitionstart",
                    OnTransitionStartEvent,
                    OnTransitionStartEventListener,
                );
                ...
                const on_drag_enter: Event =
                    ("dragenter", OnDragEnterEvent, OnDragEnterEventListener);
                const on_drag_leave: Event =
                    ("dragleave", OnDragLeaveEvent, OnDragLeaveEventListener);
                const on_drag_over: Event = ("dragover", OnDragOverEvent, OnDragOverEventListener);
                const on_drag_start: Event =
                    ("dragstart", OnDragStartEvent, OnDragStartEventListener);
                ...
                fn sub_traits() {
                    pub trait HtmlMediaElement {
                        ...
                        const on_can_play: Event =
                            ("canplay", OnCanPlayEvent, OnCanPlayEventListener);
                        const on_can_play_through: Event = (
                            "canplaythrough",
                            OnCanPlayThroughEvent,
                            OnCanPlayThroughEventListener,
                        );
                        const on_duration_change: Event = (
                            "durationchange",
                            OnDurationChangeEvent,
                            OnDurationChangeEventListener,
                        );
                        const on_emptied: Event =
                            ("emptied", OnEmptiedEvent, OnEmptiedEventListener);
                        const on_ended: Event = ("ended", OnEndedEvent, OnEndedEventListener);
                        const on_loaded_data: Event =
                            ("loadeddata", OnLoadedDataEvent, OnLoadedDataEventListener);
                        const on_loaded_metadata: Event = (
                            "loadedmetadata",
                            OnLoadedMetadataEvent,
                            OnLoadedMetadataEventListener,
                        );
                        const on_load_start: Event =
                            ("loadstart", OnLoadStartEvent, OnLoadStartEventListener);
                        const on_pause: Event = ("pause", OnPauseEvent, OnPauseEventListener);
                        const on_play: Event = ("play", OnPlayEvent, OnPlayEventListener);
                        const on_playing: Event =
                            ("playing", OnPlayingEvent, OnPlayingEventListener);
                        const on_progress: Event =
                            ("progress", OnProgressEvent, OnProgressEventListener);
                        const on_rate_change: Event =
                            ("ratechange", OnRateChangeEvent, OnRateChangeEventListener);
                        ...
                        const on_seeking: Event =
                            ("seeking", OnSeekingEvent, OnSeekingEventListener);
                        const on_stalled: Event =
                            ("stalled", OnStalledEvent, OnStalledEventListener);
                        const on_suspend: Event =
                            ("suspend", OnSuspendEvent, OnSuspendEventListener);
                        const on_time_update: Event =
                            ("timeupdate", OnTimeUpdateEvent, OnTimeUpdateEventListener);
                        const on_volume_change: Event = (
                            "volumechange",
                            OnVolumeChangeEvent,
                            OnVolumeChangeEventListener,
                        );
                        const on_waiting: Event =
                            ("waiting", OnWaitingEvent, OnWaitingEventListener);
                    }

                    pub trait HtmlFormElement {
                        const on_form_data: Event =
                            ("formdata", OnFormDataEvent, OnFormDataEventListener);

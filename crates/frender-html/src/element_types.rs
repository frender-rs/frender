...
pub trait CsrComponent<Children> {
    ...
    fn children_render_update<R: RenderHtml>(children: Children, renderer: &mut R, children_state: std::pin::Pin<&mut Self::ChildrenRenderState<R>>);

    type ChildrenUnpinnedRenderState<R: RenderHtml>: crate::RenderState<R> + Default + Unpin;

    fn children_unpinned_render_update<R: RenderHtml>(children: Children, renderer: &mut R, children_state: &mut Self::ChildrenUnpinnedRenderState<R>);
}

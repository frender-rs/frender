...
impl HookPollNextUpdate for NoData {
    #[inline(always)]
    fn poll_next_update(
        self: Pin<&mut Self>,
...
pin_project_lite::pin_project! {
    #[project = HookStateWithNoPropsProj]
...
impl<H: HookPollNextUpdate, Ctx, S> HookStateWithNoProps<H, Ctx, S> {
    pub(crate) fn pin_project(self: Pin<&mut Self>) -> HookStateWithNoPropsProj<'_, H, Ctx, S> {
        self.project()
    }
}
